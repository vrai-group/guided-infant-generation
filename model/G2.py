...
def build_model():
    ...
    conv2 = Conv2D(config.conv_hidden_num * 2, 3, (1, 1), padding='same', activation=config.activation_fn, data_format=config.data_format)(b1)
    ...
    b2 = BatchNormalization()(pool2)

    conv3 = Conv2D(config.conv_hidden_num * 3, 3, (1, 1), padding='same', activation=config.activation_fn, data_format=config.data_format)(b2)
    ...
    up4 = UpSampling2D(size=(2, 2), data_format=config.data_format, interpolation="nearest")(b3)
    ...
    up5 = UpSampling2D(size=(2, 2), data_format=config.data_format, interpolation="nearest")(b4)
    ...
    outputs = Conv2D(config.input_image_raw_channel, 1, 1, padding='same', activation='tanh', data_format=config.data_format)(b5)
