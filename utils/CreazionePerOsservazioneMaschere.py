...
def _getPoseMask(peaks, height, width, radius_head=30, radius=4, var=4, mode='Solid'):
    ...
    return dense, logs_cancellazione
...
if __name__ == '__main__':
    ...
    #intervalli_di_movimento = pd.read_csv(os.path.join('./data/BabyPose/Intervalli_di_movimento.csv'), delimiter=',')
    ...
    for name_file_annotation_0 in sorted(os.listdir(config.data_annotations_path), key = lambda x : int(x.split('_')[1].split('.')[0].split('z')[1])):
        ...
        cnt = 0
#        intervalli = intervalli_di_movimento.loc[intervalli_di_movimento['Nome'] == pz_0].values[0][1:]
        ...
        list = os.listdir('./data/nuovo dataset/BabyPose_nuovo_dataset/'+str(pz_0))

        # Creazione del pair
        for _, row_0 in pd_annotation_0.iterrows():
            # Creazione del tfrecord

            if row_0['image'] in list:
            # if (row_0['image'] == '41_8bit.png' or row_0['image'] == '381_8bit.png' or row_0['image'] == '401_8bit.png' or
            # row_0['image'] == '1131_8bit.png' or row_0['image'] == '3701_8bit.png' or row_0['image'] == '4606_8bit.png') and pz_0 == 'pz24':
        ...
        #np.save('./masks_radius_4/logs_cancellazione/{pz}.npy'.format(pz=pz_0),np.array(logs_cancellazione_immagini))
